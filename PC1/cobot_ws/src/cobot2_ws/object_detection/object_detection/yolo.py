...
class YoloModel:
    ...
    def get_best_detection(self, img_node, target):
        ...
        label_id = self._resolve_label_id(target, results[0].names)
        if label_id is None:
            print(f"Unknown target label: {target}")
            return None, None
    ...
    def _resolve_label_id(self, target, names):
        # Prefer runtime model names because external JSON can be stale.
        if isinstance(names, dict):
            for idx, name in names.items():
                if name == target:
                    return int(idx)

        # Fallback to JSON mapping for backward compatibility.
