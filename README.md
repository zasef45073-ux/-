...
- **카메라 FOV 문제로 인한 그리퍼 파지 위치 오차** — 엔드이펙터에 부착된 카메라의 시야각 문제로 컵 위치 인식이 부정확해져 그리퍼가 컵을 너무 높은 지점에서 잡는 현상 발생. 로봇팔 J6 회전으로 카메라 시야각을 조정해 해결
- **`minAreaRect` 기반 컵 방향 감지** — 컵의 회전된 형태를 정확히 인식하기 위해 최소 외접 사각형 알고리즘 적용
  
🛠️ 트러블슈팅 (담당 파트: object_detection, robot_control)
그리퍼 장착 카메라로 인한 시야각 문제 — RealSense 카메라가 로봇 팔 엔드이펙터에 장착되어 있어서, 카메라 시야가 로봇 팔의 움직임에 따라 같이 변하는 구조였습니다. 이로 인해 로봇이 컵을 향해 접근할 때 그리퍼가 컵을 실제보다 높은 위치에서 잡는 문제가 발생했습니다. 원인은 카메라가 정확한 컵 위치를 인식하지 못하고 시야각이 어긋난 상태로 좌표를 계산하고 있었던 것이었고, J6(엔드이펙터 회전 관절)를 추가로 회전시켜 카메라가 컵을 정면으로 바라보도록 각도를 보정해 해결했습니다.
컵 방향(회전) 감지 로직 — YOLO로 컵을 탐지한 후, 그리퍼가 정확히 잡을 수 있도록 컵의 회전 방향까지 파악해야 했습니다. OpenCV의 minAreaRect를 활용해 탐지된 컵의 바운딩 영역에서 최소 외접 사각형을 구하고, 이 사각형의 회전각을 컵의 방향으로 사용해 그리퍼 접근 각도를 결정하도록 구현했습니다.
